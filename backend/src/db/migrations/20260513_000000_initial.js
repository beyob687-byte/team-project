...
exports.up = async function up(knex) {
  ...
  await knex.schema.createTable("users", (table) => {
    ...
    table
      .enu("user_type", ["student", "faculty", "staff"], {
        useNative: false,
    ...
    table.unique(
      ...
      "users_university_student_id_unique",
    );
    table.unique(
      ["university_id", "external_auth_id"],
      "users_university_external_auth_unique",
    );
  ...
  await knex.schema.createTable("clubs", (table) => {
    ...
    table
      .enu("membership_policy", ["open", "approval", "invite_only"], {
        useNative: false,
    ...
    table
      .enu("status", ["pending", "active", "suspended", "inactive"], {
        useNative: false,
    ...
    table.unique(
      ["university_id", "short_name"],
      "clubs_university_short_name_unique",
    );
  ...
  await knex.schema.createTable("club_registration_requests", (table) => {
    ...
    table
      ...
      .inTable("clubs")
      .onDelete("CASCADE")
      .unique("club_registration_requests_club_unique");
    table
    ...
    table
      ...
      .defaultTo(knex.fn.now());
    table
      .enu("status", ["pending", "approved", "rejected", "conditional"], {
        useNative: false,
  ...
  await knex.schema.createTable("club_memberships", (table) => {
    ...
    table
      .enu("status", ["active", "pending", "suspended", "alumni"], {
        useNative: false,
  ...
  await knex.schema.createTable("club_role_definitions", (table) => {
    ...
    table.unique(
      ["club_id", "role_name"],
      "club_role_definitions_club_role_unique",
    );
  });
  ...
  await knex.schema.createTable("projects", (table) => {
    ...
    table
      .enu("status", ["planning", "in_progress", "completed"], {
        useNative: false,
  ...
  await knex.schema.createTable("events", (table) => {
    ...
    table.text("description").nullable();
    table
      .enu("event_type", ["in_person", "virtual", "hybrid"], {
        useNative: false,
    ...
    table.string("cover_image_url", 500).nullable();
    table
      .enu("visibility", ["public", "club_members", "invite_only"], {
        useNative: false,
    ...
    table
      .enu("status", ["draft", "published", "cancelled", "completed"], {
        useNative: false,
    ...
    table.index(
      ["club_id", "status", "start_datetime"],
      "events_club_status_start_idx",
    );
  });
  ...
  await knex.schema.createTable("event_rsvps", (table) => {
    ...
    table
      .timestamp("rsvp_submitted_at", { useTz: true })
      .notNullable()
      .defaultTo(knex.fn.now());
  ...
  await knex.schema.createTable("attendance_records", (table) => {
    ...
    table
      ...
      .onDelete("SET NULL");
    table
      .enu("checkin_method", ["manual", "qr_scan", "geo", "self_code"], {
        useNative: false,
        enumName: "attendance_checkin_method_enum",
      })
      .notNullable();
    table
      .timestamp("checkin_time", { useTz: true })
      .notNullable()
      .defaultTo(knex.fn.now());
    table
      .uuid("marked_by")
    ...
    table.unique(
      ["event_id", "user_id"],
      "attendance_records_event_user_unique",
    );
  ...
  await knex.schema.createTable("posts", (table) => {
    ...
    table
      .enu(
        "post_type",
        ["general", "event_promotion", "project_highlight", "poll"],
        {
          useNative: false,
    ...
    table.index(
      ["club_id", "status", "published_at"],
      "posts_club_status_published_idx",
    );
  });
  ...
  await knex.schema.createTable("polls", (table) => {
    ...
    table
      ...
      .inTable("posts")
      .onDelete("CASCADE")
      .unique("polls_post_unique");
    table.string("question", 500).notNullable();
    table.timestamp("expires_at", { useTz: true }).nullable();
    table
      .enu("results_visibility", ["always", "after_vote", "after_close"], {
        useNative: false,
        enumName: "poll_results_visibility_enum",
      })
      .notNullable()
      .defaultTo("after_close");
  ...
  await knex.schema.createTable("moderation_flags", (table) => {
    ...
    table
      .enu("flag_type", ["ai_auto", "user_report"], {
        useNative: false,
    ...
    table
      .enu(
        "status",
        ["pending", "resolved_approved", "resolved_rejected", "escalated"],
        {
          useNative: false,
  ...
  await knex.schema.createTable("moderation_appeals", (table) => {
    table.uuid("id").primary().defaultTo(knex.raw("uuid_generate_v4()"));
    table
      .uuid("moderation_flag_id")
      .notNullable()
      .references("id")
      .inTable("moderation_flags")
      .onDelete("CASCADE");
    table
      .uuid("appealed_by")
      .notNullable()
  ...
  await knex.schema.createTable("user_restrictions", (table) => {
    ...
    table
      ...
      .inTable("users")
      .onDelete("CASCADE")
      .unique("user_restrictions_user_unique");
    table
      .enu("restriction_type", ["post_ban", "comment_ban", "full_ban"], {
        useNative: false,
  ...
  await knex.schema.createTable("surveys", (table) => {
    ...
    table.string("title", 200).notNullable();
    table
      .enu("target_audience", ["members", "event_attendees", "all_students"], {
        useNative: false,
    ...
    table
      ...
      .onDelete("CASCADE");
    table
      .enu("status", ["draft", "published", "closed"], {
        useNative: false,
