...
const ClubProfile = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const clubData = await clubsApi.getClub(clubId);
        setClub(clubData);

        try {
          const membership = await clubsApi.getMyMembership(clubId);
  ...
  if (!club)
    return (
      <EmptyState
        ...
        description="The club you are looking for does not exist."
      />
    );
  ...
  const tabs = [
    ...
    {
      id: "projects",
      label: `Projects (${club.public_projects?.length || 0})`,
    },
    ...
    <div className="space-y-8">
        ...
        <div className="px-6 md:px-10 pb-8">
          <div className="flex flex-col md:flex-row md:items-end gap-6 -mt-16 md:-mt-20 relative z-10">
            <Avatar
              src={club.logo_url}
              alt={club.name}
              className="w-32 h-32 md:w-40 md:h-40 ring-4 ring-surface shadow-glow"
            />
            ...
            <div className="flex-1 pb-2">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-text-2">
                ...
                <div className="flex items-center gap-1.5">
                  <Calendar className="w-4 h-4" /> Est.{" "}
            ...
            <div className="flex gap-3 pb-2 w-full md:w-auto">
              {joinStatus === "sent" ? (
                ...
                >
                  {club.membership_policy === "invite_only"
                    ? "Invite Only"
                    : "Request to Join"}
                </Button>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          {activeTab === "posts" && <ClubPosts />}

          {activeTab === "about" && (
            <div className="bg-surface rounded-card border border-border-glow p-6 md:p-8 space-y-6">
              <div>
                <h3 className="text-xl font-display font-bold text-text-1 mb-3">
                  About Us
                </h3>
                  ...
                  {club.upcoming_events.map((event) => (
                    <EventCard
                      ...
                      event={{
                        ...
                        club: { name: club.name, logo: club.logo_url },
                      }}
                    />
                  ))}
                </div>
            ...
            <div className="space-y-4">
              {club.public_projects?.length > 0 ? (
                club.public_projects.map((project) => (
                  <div
                    key={project.id}
                    className="bg-surface rounded-card border border-border-glow p-6 flex justify-between items-start hover:border-primary/50 transition-colors"
                  >
                    <div>
                      <h4 className="font-bold text-text-1 mb-1">
                        {project.title}
                      </h4>
                ...
                <EmptyState
                  title="No projects"
                  description="This club doesn't have any active projects."
                />
              )}
            </div>
          )}
          ...
          {activeTab === "members" && (
            <EmptyState
              title="Member Roster"
              description="Roster visibility depends on your membership status and club privacy settings."
              icon={Users}
            />
          )}
        </div>
            ...
            <div className="space-y-4">
              {club.social_links?.website && (
                <a
                  ...
                  className="flex items-center gap-3 text-text-2 hover:text-primary transition-colors group"
                >
              ...
              {club.social_links?.instagram && (
                <a
                  href={`https://instagram.com/${club.social_links.instagram.replace("@", "")}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-3 text-text-2 hover:text-primary transition-colors group"
                >
              ...
              {club.social_links?.github && (
                <a
                  href={`https://github.com/${club.social_links.github}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-3 text-text-2 hover:text-primary transition-colors group"
                >
                  ...
                  <span className="flex-1 truncate">
                    github.com/{club.social_links.github}
                  </span>
                  <ExternalLink className="w-4 h-4 opacity-50" />
                </a>
              )}
